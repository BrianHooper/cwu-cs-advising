...
namespace PlanGenerationAlgorithm
{
    public class Class1
    {
        ...
        public static void Main()
        {
            ...
            //used variable
            Name n_name;
            n_name.FirstName = "Rico"; //student name
            n_name.LastName = "Adrian";
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                {
                    coursesTaken.Add(possibleCourses[k]);
                    schedule.removeClass(possibleCourses[k]);
                }
                if (schedule.quarterName.QuarterSeason == Season.Fall)
                {
                    schedule.quarterName.QuarterSeason = Season.Winter;
                    schedule.quarterName.Year++;
                }
                else if (schedule.quarterName.QuarterSeason == Season.Winter)
                {
                    schedule.quarterName.QuarterSeason = Season.Spring;
                }
                else if (schedule.quarterName.QuarterSeason == Season.Spring)
                {
                    schedule.quarterName.QuarterSeason = Season.Fall;
                }
            }
